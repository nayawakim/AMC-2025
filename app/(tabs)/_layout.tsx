...
function AnimatedTabBar(props: BottomTabBarProps)  {
  ...
  useEffect(() => {
    const toValue = state.index * tabWidth + (tabWidth - INDICATOR_WIDTH) / 2;
...
if (typeof options.tabBarLabel === "string") {
  ...
  label = route.name;
        ...
        if (route.name === "map") {
          ...
          source = isFocused ? icons.chat.active : icons.chat.inactive;
        } else if (route.name === "scan") {
          source = isFocused ? icons.scan.active : icons.scan.inactive;
        } else {
          source = undefined;
        }
